...
"""Grouped TurboQuant key quantize-and-store kernels.

This is the official-mode writer for grouped TurboQuant keys. It keeps the
legacy flat TurboQuant kernels untouched and only handles the grouped key
payload; values are still packed by the existing uniform-value path.
"""
...
def turboquant_write_packed_kv(
    ...
    """Quantize raw grouped TurboQuant keys and write them into cache."""
    if x.numel() == 0:
        return
    if x.device.type != "cuda":
        raise ValueError("Grouped TurboQuant Triton KV update requires CUDA tensors.")
    if cache.dtype != torch.uint8:
        raise ValueError("TurboQuant grouped key cache expects uint8 storage.")
    if x.ndim != 3:
