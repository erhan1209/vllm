...
@triton.jit
def _grouped_unpack_fixed_indices(
    cache_ptr,
    token_base,
    offs_d,
    ...
    indices = tl.zeros([BLOCK_KV, HEAD_SIZE_PADDED], dtype=tl.int32)
    if bits == 0:
        return indices
...
def triton_turboquant_decode_attention(
    ...
    if grouped_recipe is not None:
        ...
        if query.device.type == "cuda":
            ...
            kernel_meta = get_turboquant_kernel_meta(query.device, D)
            group0, group1 = layout.groups
            q_head_to_kv = (
                torch.arange(Hq, device=query.device, dtype=torch.int64) // kv_group_size
            )
            q_rot, q_qjl = apply_turboquant_query_transforms(
                query=query,
                group_indices=group_indices,
                rotations=group_rotations,
                qjl_matrices=group_qjl,
                kv_head_for_query_head=q_head_to_kv,
            )
            ...
            _grouped_tq_decode_stage1[grid](
                q_rot[0],
                q_qjl[0],
                q_rot[1],
                q_qjl[1],
                ...
                block_table,
                seq_lens,
                group_centroids[group0.mse_bits],
                group_centroids[group1.mse_bits],
                mid_o,
                q_rot[0].stride(0),
                q_rot[0].stride(1),
                q_rot[1].stride(0),
                ...
                BLOCK_KV=4,
                G0_DIM=group0.dim,
                G0_PADDED=triton.next_power_of_2(group0.dim),
                ...
                G0_QJL_SCALE=math.sqrt(math.pi / 2.0) / group0.dim,
                G1_DIM=group1.dim,
                G1_PADDED=triton.next_power_of_2(group1.dim),
                ...
                G1_QJL_SCALE=math.sqrt(math.pi / 2.0) / group1.dim,
