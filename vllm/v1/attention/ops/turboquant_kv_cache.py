...
TURBOQUANT_COMPAT_DTYPES = {
    "turboquant25": "turboquant25",
    "turboquant35": "turboquant35",
    "turboquant_3bit": "turboquant25",
    "turboquant_4bit": "turboquant35",
}
...
def canonicalize_turboquant_dtype(bits_or_dtype: float | int | str) -> str:
    if isinstance(bits_or_dtype, str):
        try:
            return TURBOQUANT_COMPAT_DTYPES[bits_or_dtype]
...
def _layout_cached(kv_cache_dtype: str, head_size: int) -> TurboQuantLayout:
    recipe = canonicalize_turboquant_dtype(kv_cache_dtype)
    group_dims = get_turboquant_group_dims(head_size, recipe)
    group_bits = TURBOQUANT_GROUP_BITS[recipe]
...
def get_turboquant_packed_dim(head_size: int, bits_or_dtype: float | int | str) -> int:
    return get_turboquant_layout(
        canonicalize_turboquant_dtype(bits_or_dtype), head_size
    ).packed_dim
...
def _structured_signs_cached(
    ...
    randint_kwargs: dict[str, Any] = {"generator": generator, "device": device}
    if device.type == "cuda":
        with torch.accelerator.device_index(device.index):
            draws = torch.randint(0, 2, (dim,), **randint_kwargs)
    else:
        draws = torch.randint(0, 2, (dim,), **randint_kwargs)
    return torch.where(
        draws > 0,
...
def get_turboquant_rotation(
    ...
    return _structured_signs_cached(
        device.type, device.index, dim, TURBOQUANT_SEED + seed_offset + dim
    )
...
def get_turboquant_mse_transform_matrix(
    ...
    return _transform_matrix_cached(
        device.type, device.index, dim, seed_offset, "mse_forward"
    )
...
def get_turboquant_mse_inverse_transform_matrix(
    ...
    return _transform_matrix_cached(
        device.type, device.index, dim, seed_offset, "mse_inverse"
    )
...
def get_turboquant_qjl_transform_matrix(
    ...
    return _transform_matrix_cached(
        device.type, device.index, dim, seed_offset, "qjl_forward"
    )
...
def get_turboquant_qjl_inverse_transform_matrix(
    ...
    return _transform_matrix_cached(
        device.type, device.index, dim, seed_offset, "qjl_inverse"
    )
...
def _bit_layout(
    ...
    return flat_positions // 8, flat_positions % 8


def pack_turboquant_indices(indices: torch.Tensor, bits: int) -> torch.Tensor:
    if bits <= 0:
        shape = (*indices.shape[:-1], 0)
        return torch.empty(shape, dtype=torch.uint8, device=indices.device)
    head_size = indices.shape[-1]
    num_bytes = (head_size * bits + 7) // 8
    byte_idx, bit_shift = _bit_layout(
        indices.device.type, indices.device.index, head_size, bits
    )
    bits_view = torch.zeros(
        (*indices.shape[:-1], num_bytes), dtype=torch.uint8, device=indices.device
    )
    expanded = indices.to(torch.int64).unsqueeze(-1)
    offsets = torch.arange(bits, dtype=torch.int64, device=indices.device)
    expanded_bits = ((expanded >> offsets) & 1).reshape(*indices.shape[:-1], -1)
    bits_view.scatter_add_(
        -1,
        byte_idx.reshape(*((1,) * (indices.ndim - 1)), -1).expand_as(expanded_bits),
        (expanded_bits << bit_shift.reshape(*((1,) * (indices.ndim - 1)), -1)).to(
            torch.uint8
        ),
    )
    return bits_view
...
def unpack_turboquant_indices(
    ...
    if bits <= 0:
        shape = (*packed.shape[:-1], head_size)
        return torch.zeros(shape, dtype=torch.uint8, device=packed.device)
    byte_idx, bit_shift = _bit_layout(
        packed.device.type, packed.device.index, head_size, bits
    )
    gathered = packed.index_select(-1, byte_idx).to(torch.int64)
    gathered = gathered.reshape(*packed.shape[:-1], head_size, bits)
    shifts = bit_shift.reshape(*((1,) * (packed.ndim - 1)), head_size, bits)
    values = ((gathered >> shifts) & 1) << torch.arange(
        bits, dtype=torch.int64, device=packed.device
    ).reshape(*((1,) * (packed.ndim - 1)), 1, bits)
    return values.sum(dim=-1).to(torch.uint8)
...
def validate_turboquant_group_indices(
    x: torch.Tensor,
    group_indices: tuple[torch.Tensor, torch.Tensor],
) -> None:
    if group_indices[0].shape[0] != x.shape[1] or group_indices[1].shape[0] != x.shape[1]:
...
def scatter_turboquant_output(
    ...
    for group_output, indices in zip(group_outputs, group_indices, strict=True):
        output.scatter_add_(
            ...
            group_output.to(dtype=output.dtype),
        )
    if out is not None:
        return out
...
def quantize_turboquant_vectors(
    ...
    validate_turboquant_group_indices(x, group_indices)
    ...
    for group_x, group_layout, rotation, qjl_matrix in zip(
        groups, layout.groups, rotations, qjl_matrices, strict=True
    ):
        ...
        mse_indices = torch.zeros_like(rotated, dtype=torch.uint8)
        rotated_hat = torch.zeros_like(rotated, dtype=torch.float32)
        if group_layout.mse_bits > 0:
            group_centroids = centroids[group_layout.mse_bits]
            mse_indices = torch.abs(rotated.unsqueeze(-1) - group_centroids).argmin(
                dim=-1
            ).to(torch.uint8)
            rotated_hat = group_centroids[mse_indices.long()]
        ...
        packed_groups.append(
            torch.cat(
                (
                    pack_turboquant_indices(mse_indices, group_layout.mse_bits),
                    pack_turboquant_indices(qjl_bits, 1),
                    _norms_to_bytes(
                        vector_norms.squeeze(-1), TURBOQUANT_VECTOR_NORM_BYTES
                    ),
                    _norms_to_bytes(
                        residual_norms.squeeze(-1), TURBOQUANT_RESIDUAL_NORM_BYTES
                    ),
                ),
                dim=-1,
            )
...
def dequantize_turboquant_vectors(
    ...
    if packed.shape[-2] != group_indices[0].shape[0]:
        raise ValueError("TurboQuant packed tensor KV head count does not match metadata.")
    ...
    for group_layout, rotation, qjl_matrix in zip(
        layout.groups, rotations, qjl_matrices, strict=True
    ):
